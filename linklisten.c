...
__interrupt void Timer_A (void)
{
  switch (gameStatus){
    ...
    case SINGLE_PLAYER:
      ...
      break;
    case MULTIPLAYER:
        if(timerCnt !=20){                                  // time before timeout
          TACTL = MC_0 + TACLR;
          InitTimerA(4);
          timerCnt++;
        }
...
__interrupt void USCI0RX_ISR(void)
{
  ...
  switch(gameStatus){
    case SINGLE_PLAYER:
      ...
      else{                                             // get first 3 digits of player answer as long as timeOut didn't occure
        if(!timeOut){
    ...
    case MULTIPLAYER:
      ...
      break;
    case MENU:
      if(gotName){                                      // get player desicion for single/multiplayer game mode
        if(rx == '1') {
          __bic_SR_register_on_exit(LPM3_bits);
          SinglePlayer();
          TACTL = MC_0 + TACLR;
   ...
   case CHOOSE_DIFF:
     if(isWaitingForAnswer){
       switch(rx-48){                                   // get player desicion for game difficulty level
            case EASY:
                timeOutLatency=4;
                gameGlobaldiff=1;
                __bic_SR_register_on_exit(LPM3_bits);
...
void TwoPlayer()
{
  ...
  while (1)
  {
    if (SMPL_SUCCESS == SMPL_LinkListen(&linkID))
    {
      break;
    }
    ...
    SMPL_Send(linkID, (uint8_t*)usrName, 5);  // send host player name

    while(!sPeerFrameSem);
    SMPL_Receive(linkID, otherusrName, &len);  // get guest player name

    BSP_ENTER_CRITICAL_SECTION(intState);
    sPeerFrameSem--;
    BSP_EXIT_CRITICAL_SECTION(intState);

    num[0]='0'+ gameGlobaldiff;
    SMPL_Send(linkID,num,1);   // send guest player the chosen difficulty level
  ...
  while(1)
  {
    ...
    num[0]= myScore;
    SMPL_Send(linkID, num, 1);  // send host player score

    while(!sPeerFrameSem);
    SMPL_Receive(linkID, rxmsg, &len); // get guest player score
    BSP_ENTER_CRITICAL_SECTION(intState);
    sPeerFrameSem--;
    BSP_EXIT_CRITICAL_SECTION(intState);
